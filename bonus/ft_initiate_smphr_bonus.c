...
/*   Updated: 2024/06/11 17:13:37 by bramzil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_smphr_unlink(ph_t *phls)
{
	...
	(sem_unlink("/die_smphr"), sem_unlink("/msg_smphr"));
	(sem_unlink("/prnt_smphr"), sem_unlink("/forks_smphr"));
...
{
	(0 < flg) && (sem_close(glb->die_smphr), \
		sem_unlink("/die_smphr"));
	(1 < flg) && (sem_close(glb->msg_smphr), \
		sem_unlink("/msg_smphr"));
	(2 < flg) && (sem_close(glb->prnt_smphr), \
		sem_unlink("/prnt_smphr"));
	(3 < flg) && (sem_close(glb->forks_smphr), \
		sem_unlink("/forks_smphr"));
	(4 < flg) && (sem_close(glb->syn_die_smphr), \
		sem_unlink("/syn_die_smphr"));
	return (0);
}
...
int ft_initiate_semaphores(ph_t *phls, glb_t *glb)
{
	...
	if (ft_open_smphr(glb->die_smphr, "/die_smphr", 1))
